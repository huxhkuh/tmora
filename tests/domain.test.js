...
test("recent projects preserve empty-segment handling and stable ties", () => {
  const s = fixture();
  assert.deepEqual(recentProjects(s, 4).map((p) => p.id), ["p1", "p2"]);
  s.entries = [
    { projectId: "p1", segments: [] },
    { projectId: "p2", segments: [{ start: 1000, end: 2000 }] },
  ];
  assert.deepEqual(recentProjects(s, 4).map((p) => p.id), ["p2", "p1"]);
  s.entries.push({ projectId: "p1", segments: [{ start: 1000, end: 2000 }] });
  assert.deepEqual(recentProjects(s, 4).map((p) => p.id), ["p1", "p2"]);
});
